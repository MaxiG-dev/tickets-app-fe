...
const Users = ({ user }: any) => {
  ...
  const handleOkSearchModal = async () => {
    ...
    if (validForm) {
      const fieldForms = {
        purchaseNumber: searchForm.getFieldsValue()['Purchase number'],
        email: searchForm.getFieldsValue()['Email'],
        filterParam: searchForm.getFieldsValue()['Filter keywords'],
        status: searchForm.getFieldsValue()['Status'],
        includeDeleted: searchForm.getFieldsValue()['Is deleted'],
      };
      setOpenSearchModal(false);
      await getTickets(fieldForms);
    } else {
